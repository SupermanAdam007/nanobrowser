...
/**
 * Convert input messages to a format compatible with the target model.
 * Some models (e.g. deepseek-reasoner) require strictly alternating user/assistant messages.
 */
export function convertInputMessages(inputMessages: ModelMessage[], modelName: string | null): ModelMessage[] {
  if (modelName === null) return inputMessages;

  if (modelName === 'deepseek-reasoner' || modelName.includes('deepseek-r1')) {
    let merged = mergeConsecutiveByRole(inputMessages, 'user');
    merged = mergeConsecutiveByRole(merged, 'assistant');
    return merged;
  }
...
function mergeConsecutiveByRole(messages: ModelMessage[], role: 'user' | 'assistant'): ModelMessage[] {
  const result: ModelMessage[] = [];

  for (const message of messages) {
    if (message.role === role && result.length > 0 && result[result.length - 1].role === role) {
      const last = result[result.length - 1];
      const lastContent = typeof last.content === 'string' ? last.content : '';
      const msgContent = typeof message.content === 'string' ? message.content : '';

      if (role === 'user') {
        (result[result.length - 1] as UserModelMessage).content = `${lastContent}\n${msgContent}`;
      } else {
        (result[result.length - 1] as AssistantModelMessage).content = `${lastContent}\n${msgContent}`;
      }
    } else {
